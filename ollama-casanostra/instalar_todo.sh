...
# ------------------------------------------------- 0. Asegurar curl (Ubuntu)
if ! command -v curl >/dev/null 2>&1 && command -v apt-get >/dev/null 2>&1; then
  echo "Instalando curl (necesario para descargar Ollama)..."
...
cat > conocimiento/programacion.md <<'FIN_DE_ARCHIVO'
...
cat > conocimiento/ingenieria_de_prompts.md <<'FIN_DE_ARCHIVO'
...
cat > conocimiento/bucles_agenticos.md <<'FIN_DE_ARCHIVO'
...
cat > conocimiento/resolucion_de_problemas.md <<'FIN_DE_ARCHIVO'
...
cat > preguntas_programacion.json <<'FIN_DE_ARCHIVO'
[
...
cat > preguntas_soluciones.json <<'FIN_DE_ARCHIVO'
[
...
# Indexar los temarios en la biblioteca RAG (no bloquea si falla)
(cd "$DIR" && python3 biblioteca.py indexar conocimiento) || \
  echo "  ⚠ No se pudo indexar el conocimiento (¿falta python3?); hazlo luego con: python3 $DIR/biblioteca.py indexar $DIR/conocimiento"
...
echo "  python3 $DIR/examen.py preguntas_programacion.json casanostra   → examen de programación"
echo "  python3 $DIR/examen.py preguntas_soluciones.json casanostra resolutor  → examen de soluciones"
echo ""
